...
let myDataChannel; // RTCDataChannel 객체를 저장하는 변수 (필요한 경우)
...
socket_IO.on("welcome2", async (userNickname, newCount) => {
  ...
  myDataChannel = myPeerConnection.createDataChannel("chat"); // 데이터 채널 생성 (필요한 경우)
  myDataChannel.addEventListener("message", (event) => {
    console.log(`Received message from data channel: ${event.data}`);
    addMessage(`Peer: ${event.data}`); // 데이터 채널을 통해 받은 메시지를 채팅에 추가
  });
...
myPeerConnection.createDataChannel('test');
myPeerConnection.createOffer().then(offer => pc.setLocalDescription(offer));
...
async function handleCameraChange() {
  ...
  if (myPeerConnection) {
    ...
    if (sender) {
      sender.replaceTrack(videoTrack); // RTCPeerConnection의 비디오 트랙 교체
    } else {
...
socket_IO.on("offer", async (offer) => {
  ...
  myPeerConnection.addEventListener("datachannel", (event) => {
    console.log(`Data channel event received: ${event.channel.label}`);
